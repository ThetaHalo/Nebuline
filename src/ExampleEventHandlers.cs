...
public static class ExampleEventHandlers
{
    public static void Initialize()
    {
        ...
        {
            Logger<NebulinePlugin>.Info($"{@event.Source.Data.PlayerName} is about to kill {@event.Target.Data.PlayerName}");
        });

        MiraEventManager.RegisterEventHandler<AfterMurderEvent>(@event =>
        {
            Logger<NebulinePlugin>.Info($"{@event.Source.Data.PlayerName} has killed {@event.Target.Data.PlayerName}");
        });

        MiraEventManager.RegisterEventHandler<CompleteTaskEvent>(@event =>
        {
            Logger<NebulinePlugin>.Info($"{@event.Player.Data.PlayerName} completed {@event.Task.TaskType.ToString()}");
        });
    ...
    {
        Logger<NebulinePlugin>.Info($"{@event.Button.buttonLabelText.text} is being used!");
        if (@event.PlayerTarget != null)
        {
            Logger<NebulinePlugin>.Info($"{@event.PlayerTarget.Data.PlayerName} is being targeted!");
        }
        else if (@event.VentTarget != null)
        {
            Logger<NebulinePlugin>.Info($"Vent with id {@event.VentTarget.Id.ToString(CultureInfo.InvariantCulture)} is being targeted!");
        }
    }
    ...
    {
        Logger<NebulinePlugin>.Error(@event.SystemType.ToString());
    }
    ...
    {
        Logger<NebulinePlugin>.Warning("Freeze button clicked!");
    ...
    {
        Logger<NebulinePlugin>.Warning("Freeze button cancelled!");
